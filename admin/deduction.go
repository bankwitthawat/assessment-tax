...
func GetDeductionPersonal() (MasDeductions, error) {
...
func CreateDeductionPersonal(amount float64) (MasDeductions, error) {
	result := MasDeductions{}

	row := dbConfig.DB.QueryRow("INSERT INTO mas_deductions (type, amount) values ($1, $2)  RETURNING id, type, amount", "personal", amount)
	err := row.Scan(&result.ID, &result.Type, &result.Amount)
	if err != nil {
...
func UpdateDeductionPersonal(u MasDeductions) error {
